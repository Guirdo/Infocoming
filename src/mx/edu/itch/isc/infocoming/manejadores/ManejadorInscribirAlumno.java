...
public class ManejadorInscribirAlumno implements ActionListener, WindowListener, ListSelectionListener {
    ...
    private void consultaHorarios() throws SQLException {
        Object[][] datos = intBD.consultar("select horario, tipocurso from grupo, curso where curso=idcurso");
        dm.tabla.setModel(new DefaultTableModel(datos,new Object[]{"Horario", "Curso"}));    
    }
    ...
    private void inscribir() throws SQLException {
        ...
        tel= dm.tel.getText();
//        grupo= dm.tabla.getSelectionModel();
        intBD.procedimientoInsertar("{call insertarAlumno(nombreAlumno, apellidoPaternoAlumno,apellidoMaternoAlumno,domicilio,telefono,grupo)}","", "Ramirez", "Contreras", "Col. Ye", "3435363733", 2);}
    ...
    public void valueChanged(ListSelectionEvent e) {
        if (e.getSource() == dm.tabla.getSelectionModel()) {
            ...
            horarioselect = (String) dm.tabla.getValueAt(dm.tabla.getSelectedRow(), 0);
            try {
    ...
    private void consultarGrupo(String horarioselect) throws SQLException {
        intBD.consultar("select idGrupo from grupo where horario="+"Jueves 3-6:30"+"");
